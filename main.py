...
async def transcribe(file: UploadFile = File(...)):
    ...
    if message_content is None:
        raise HTTPException(status_code=502, detail="La transcripción falló, probá con otra foto")

    clean_text = clean_json(message_content)
    final_text = json.loads(clean_text)

    return {"contenido": final_text["contenido"]}

@app.post('/epub')
async def build_epub(contenido: List[str] = Form(...), book_title: str = Form("Fragmento AirLib")):
    buffer = await create_epub(contenido, book_title)
